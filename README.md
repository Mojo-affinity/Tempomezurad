...
X.509のDER形式と、1件以上の証明書を含むPEM形式に対応しています。
接続設定の「接続診断」では、証明書形式、TLS接続、ログイン遷移を確認できます。
診断ログに企業ID、従業員番号、パスワード、URLのクエリ文字列は記録されません。
クライアント証明書認証が必要な場合、秘密鍵を含まない `.cer` 単体では認証できません。
